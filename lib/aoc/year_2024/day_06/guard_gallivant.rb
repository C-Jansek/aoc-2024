  ...
  module Year2024
    class Day06
      ...
      def parse(input)
        ...
        {
          obstructions: Set.new(obstructions),
          guard: guard,
      ...
      def part_two(input)
        ...
        visited_positions.uniq do |potential|
          potential[0..1]
        end.count do |potential|
          next false if potential[0..1] == guard_position

          true == walk_path(guard_position, guard_direction, obstructions + [potential[0..1]], width, height)
        end
      end
    end
  end
